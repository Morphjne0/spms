...
public class StudentManagePanel extends JPanel implements ActionListener {
	...
	String header[] = {"학번", "성명", "생년월일", "학과", "학급","제적상태"};
	String[] AdOption = { "학생등록", "학생정보수정", "학생제적", "제적해제", "학생삭제"};
	String[] comboName = { "ALL", "일반", "제적"};
	...
	public StudentManagePanel() {
		...
		fccombo = new JComboBox(comboName);
		fccombo.setFont(FontList.default14);
		GridBagConstraints gbc_combo = new GridBagConstraints();
		gbc_combo.fill = GridBagConstraints.BOTH;
		gbc_combo.insets = new Insets(0, 0, 0, 5);
		gbc_combo.gridx = 3;
		gbc_combo.gridy = 0;
		panel.add(fccombo, gbc_combo);
		//combo.setBounds(800, 0, 80, 30);
		...
		setList(DB_Manager.getmemberlist("S"));
		/*
	...
	public static void setList(ArrayList<Person> list) {
		...
		for(Person p : list) {
			String rj="";
			if(p.getType().equals("SR")) {rj="제적됨";}
			Object[] temp= {p.getId(),p.getName(),p.getBirth(),p.getDepart(),p.getGrade(),rj};
			model.addRow(temp);
		}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == confirmBtn) {
			...
			if (Adminfunc.equals(AdOption[0])) {
				new stdManageJDialog(AdOption[0]);
			} else if (Adminfunc.equals(AdOption[1])) {
				stdManageJDialog jd= new stdManageJDialog(AdOption[1]);
	            int row = table.getSelectedRow();
	            String id=table.getValueAt(row, 0).toString();
	            String name = table.getValueAt(row, 1).toString();
	            String birth = table.getValueAt(row, 2).toString();
	            String depart = table.getValueAt(row, 3).toString();
				Person p = new Person(id, name, birth, depart, "S");
				p.setGrade(Integer.parseInt(table.getValueAt(row, 4).toString()));
				jd.setField(p);

			}else if(Adminfunc.equals(AdOption[2])){//제적
				int row = table.getSelectedRow();
	            String id=table.getValueAt(row, 0).toString();
	            int value = OptionDialog.getInstance().question(1);
	            if(value!=-1) DB_Manager.rejectStudent(id);
	            setList(DB_Manager.getmemberlist("S"));
	            
			}else if(Adminfunc.equals(AdOption[3])){//제적해제
				int row = table.getSelectedRow();
	            String id=table.getValueAt(row, 0).toString();
	            int value = OptionDialog.getInstance().question(2);
	            if(value!=-1) DB_Manager.unrejectStudent(id);
	            setList(DB_Manager.getmemberlist("S"));
	            
			} else if (Adminfunc.equals(AdOption[4])) {// 삭제 메뉴아이템 클릭
				...
				if (DB_Manager.DeleteMember(obj.toString()) > 0) {
					stdManageJDialog.messageBox(this, "레코드 삭제되었습니다.");
					setList(DB_Manager.getmemberlist("S"));
				} else {
					stdManageJDialog.messageBox(this, "레코드가 삭제되지 않았습니다.");
				}
			}
			...
			String searchfunc = fccombo.getSelectedItem().toString();
			String searchopt="";
				switch(searchfunc) {
				case "ALL":
					searchopt="S' or type='SR";
					break;
				case "일반":
					searchopt="S";
					break;
				...
				if (TextField_SCH.getText().isBlank()) {
					...
					setList(DB_Manager.getmemberlist(searchopt));
				} else {// 검색어를 입력했을경우
					setList(DB_Manager.memberSearch(TextField_SCH.getText(),searchopt));
				}

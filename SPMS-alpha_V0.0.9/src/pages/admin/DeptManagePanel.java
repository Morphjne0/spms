...
public class DeptManagePanel extends JPanel implements ActionListener {

	// deptManageDAO dao = new deptManageDAO();

	deptManageJDialog dmj;

	static DefaultTableModel model;

	String fieldName;

	String header[] = { "학과번호", "학과명", "대표교수", "학생수" };
	String[] AdOption = { "학과생성", "학과수정", "학과삭제"};
	// String[] comboName = { "ALL", "depcode", "depname", "representprof" };

	JComboBox<String> Adcombo;
	...
	public DeptManagePanel() {
		...
		setList(DB_Manager.getdepartlist());
		/*
	...
	public static void setList(ArrayList<Department> list) {
		model.setRowCount(0);
		for (Department d : list) {
			Object[] temp = { d.getDepartId(), d.getDepartName(), d.getDepartRepresent(), d.getStudents() };
			model.addRow(temp);
		}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == confirmBtn) {
			...
			if (Adminfunc.equals(AdOption[0])) {
				new deptManageJDialog(AdOption[0]);
			} else if (Adminfunc.equals(AdOption[1])) {
				deptManageJDialog jd = new deptManageJDialog(AdOption[1]);
				int row = table.getSelectedRow();
				String id = table.getValueAt(row, 0).toString();
				String name = table.getValueAt(row, 1).toString();
				String represent = table.getValueAt(row, 2).toString();
				int stds = Integer.parseInt(table.getValueAt(row, 3).toString());
				Department d = new Department(id, name, represent, stds);
				jd.setField(d);
				...
				if (DB_Manager.deptDelete(obj.toString()) > 0) {
					deptManageJDialog.messageBox(this, "레코드 삭제되었습니다.");
					setList(DB_Manager.getdepartlist());

				} else {
					deptManageJDialog.messageBox(this, "레코드가 삭제되지 않았습니다.");
				}
			}
			...
			if (TextField_SCH.getText().isBlank()) {
				...
				setList(DB_Manager.departSearch(TextField_SCH.getText()));
			}
